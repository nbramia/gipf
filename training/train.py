"""
Training loop for Yinsh value / policy-value network.

Usage:
  python training/train.py --data data/train.ndjson --epochs 30 --output training/best.pt
  python training/train.py --data data/new.ndjson --checkpoint training/v1.pt --epochs 30 --output training/v2.pt
  python training/train.py --data data/new.ndjson --data-append data/old.ndjson --merge-ratio 0.5 --output training/v2.pt
  python training/train.py --data data/new.ndjson --model-type policy-value --checkpoint training/v67.pt --output training/v68.pt
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train Yinsh value network")
    parser.add_argument("--data", required=True, help="Path to NDJSON training data")
    parser.add_argument("--epochs", type=int, default=30)
    parser.add_argument("--batch-size", type=int, default=256)
    parser.add_argument("--lr", type=float, default=1e-3)
    parser.add_argument("--output", default="training/best.pt")
    ...
    # Model
    if use_policy:
        model = YinshPolicyValueNet().to(device)
    else:
        model = YinshValueNet().to(device)
