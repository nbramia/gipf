...
def run_epoch(model, loader, device, opt, policy_weight):
    train = opt is not None
    model.train(train)
    total_value, total_policy, batches = 0.0, 0.0, 0
    for batch in loader:
        x, policy, _winner, heuristic = [t.to(device) for t in batch]
        with torch.set_grad_enabled(train):
            value_scalar, policy_logits = model(x)
            # Value: MSE regression to heuristic eval (shape [B,1] vs [B])
            v_loss = F.mse_loss(value_scalar.squeeze(1), heuristic)
...
def main():
    ...
    for epoch in range(args.epochs):
        tr_v, tr_p = run_epoch(model, train_loader, device, opt, args.policy_weight)
        va_v, va_p = run_epoch(model, val_loader, device, None, args.policy_weight)
