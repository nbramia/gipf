...
test('claim aggregate budget stops before another command can exceed the handler deadline',async()=>{
  ...
  globalThis.fetch=async(url,options)=>{
    // Three preflight commands remain; simulate 2.9s CPU/serialization after MGET.
    commands++;const result=await original(url,options);elapsed+=2900;
    if(JSON.parse(options.body)[0]==='MGET') elapsed+=2900;
    return result;
  };
  try {
    assert.equal((await call({action:'claim',legacyId:legacy})).statusCode,503);
    assert.equal(commands,4);assert.equal(redis('GET',key),null);assert.equal(redis('GET',claimKey),null);
...
test('exhausted claim races and lifetime limit mutate neither destination nor ownership',async()=>{
  ...
  assert.equal(races,6);assert.equal(redis('GET',key),last);assert.equal(redis('GET',claimKey),null);
...
test('maximum-count synthetic domains and five alternatives preserve exact CAS at measured payload sizes',async()=>{
  ...
  globalThis.fetch=async(url,options)=>{
    const args=JSON.parse(options.body);
    if(args[0]==='EVAL' && args[2]===8) claimBytes=Math.max(claimBytes,Buffer.byteLength(options.body));
...
test('claim retry budget includes preflight and previous attempts',async()=>{
  ...
  globalThis.fetch=async(url,options)=>{
    ...
    const result=await original(url,options);elapsed+=2350;return result;
  };
  try {
    assert.equal((await call({action:'claim',legacyId:legacy})).statusCode,503);
