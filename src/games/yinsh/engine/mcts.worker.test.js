...
jest.mock('./mcts', () => ({ __esModule: true, default: jest.fn() }));
jest.mock('../YinshBoard', () => ({ __esModule: true, default: {
...
beforeEach(() => {
  ...
  getBestMove = jest.fn().mockResolvedValue({ move: [0, 0], rootNode: { uncloneable() {} } });
  ...
  jest.isolateModules(() => {
    ...
    jest.doMock('./mcts', () => ({ __esModule: true, default: MCTS }));
    require('./mcts.worker');
    handler = self.onmessage;
  });
  self.postMessage = jest.fn();
});
const compute = (requestId, modelPath = '/gipf/models/yinsh-value-v1.onnx') =>
...
test('successful network is cached and results echo request IDs and actual mode', async () => {
  ...
  expect(load).toHaveBeenCalledWith('/gipf/models/yinsh-value-v1.onnx');
