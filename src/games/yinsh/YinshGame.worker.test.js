...
jest.mock('./engine/mcts', () => ({ __esModule: true, default: jest.fn() }));
...
beforeEach(() => {
  ...
  localStorage.setItem('yinshDifficulty', 'expert');
  workers = [];
...
function reply(request, type = 'result', mode = 'nn', destination = [0, 0]) {
  act(() => request.worker.onmessage({ data: {
    requestId: request.requestId, type, data: { move: null, destination, type: 'place-ring' },
...
test.each(['toolbar', 'keyboard'])('%s undo and redo invalidate pending work and preserve the restored board', method => {
  ...
  // YINSH reset clears its initial history; a second placement provides undo.
  reply(request(), 'result', 'nn', [1, 0]);
  expect(applyAIMove).toHaveBeenCalledTimes(2);
...
test('fallback is visible and successful NN clears it without changing difficulty preference', () => {
  ...
  expect(localStorage.getItem('yinshDifficulty')).toBe('expert');
  reply(request());
...
test.each(['result', 'error'])('main-thread fallback ignores a delayed %s after reset while a new request runs', async outcome => {
  ...
  const move = { move: null, destination: [0, 0], type: 'place-ring' };
  await act(async () => {
