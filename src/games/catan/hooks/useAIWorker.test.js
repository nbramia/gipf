...
test('passes Catan search budgets with each request identity', () => {
  const { result } = renderHook(() => useAIWorker());
  act(() => result.current.computeMove({ phase: 'roll' }, 10, jest.fn(), jest.fn(), 44, 24));
  expect(workers[0].postMessage.mock.calls[0][0]).toEqual({ type: 'compute', requestId: 1,
    data: { boardState: { phase: 'roll' }, simulations: 10, maxChildren: 44, rolloutSteps: 24 } });
});
